...
module "iam" {
  ...
  role_name               = "${var.project_name}-dev-eks-load-balancer-controller"
...
# Secrets Manager functionality is now integrated into the RDS module
...
module "vpc_peering" {
  ...
  name = "${var.project_name}-dev-to-shared"
  ...
  # Security group rules - EKS security group will be added after cluster creation
  requester_security_group_id = "" # Will be updated after EKS cluster is created
...
# Application Security Group for dev environment workloads
resource "aws_security_group" "dev_app" {
  name_prefix = "${var.project_name}-dev-app-"
  vpc_id      = module.vpc.vpc_id
  description = "Security group for dev environment application workloads - allows outbound traffic to all destinations"
...
module "rds" {
  ...
  # Password is auto-generated and stored in Secrets Manager by the RDS module
...
module "elasticsearch" {
  ...
  name          = "${var.project_name}-dev-elasticsearch"
  description   = "Security group for Elasticsearch cluster"
  vpc_id        = module.vpc.vpc_id
  subnet_id     = module.vpc.private_subnets[0] # Launch in first private subnet
